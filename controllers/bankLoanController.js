...
const updateApplication = async (req, res, next) => {
  try {
    ...
    const allowedFields = ['loanAmount', 'loanScheme', 'loanType', 'bankRefNumber', 'currentStage',
      'applicationDate', 'applicationSubmissionDate', 'approvalDate', 'disbursementDate', 'priority'];
...
// PUT /api/bank-loans/:id/loan-preparation
const updateLoanPreparation = async (req, res, next) => {
  try {
    const app = await BankLoanApplication.findOne({ _id: req.params.id, isDeleted: false });
    if (!app) return next(ApiError.notFound('Application not found.'));
    ...
    const allowed = ['preparationStatus', 'preparationStartDate', 'preparationCompletedDate', 'loanAmountCalculated'];
    allowed.forEach(f => { if (req.body[f] !== undefined) app.loanPreparation[f] = req.body[f]; });
    app.markModified('loanPreparation');

    if (req.body.preparationStatus && req.body.preparationStatus !== prevStatus) {
      app.timeline.push({
        activity: `Loan preparation status changed to "${req.body.preparationStatus}"`,
        activityType: 'note',
        performedBy: req.user._id,
        previousStatus: prevStatus,
        newStatus: req.body.preparationStatus,
        isSystemGenerated: true,
      });
    }

    await app.save();
    return ApiResponse.success(res, 'Loan preparation saved', app.loanPreparation);
  } catch (err) {
    next(err);
  }
};

// PUT /api/bank-loans/:id/loan-sanction
const updateLoanSanction = async (req, res, next) => {
  try {
    const app = await BankLoanApplication.findOne({ _id: req.params.id, isDeleted: false });
    if (!app) return next(ApiError.notFound('Application not found.'));

    const prevStatus = app.loanSanction?.sanctionStatus;
    if (!app.loanSanction) app.loanSanction = {};
    const allowed = ['sanctionStatus', 'sanctionDate', 'sanctionedAmount', 'sanctionLetterNumber', 'sanctionConditions', 'rejectionReason'];
    allowed.forEach(f => { if (req.body[f] !== undefined) app.loanSanction[f] = req.body[f]; });
    app.markModified('loanSanction');

    if (req.body.sanctionStatus && req.body.sanctionStatus !== prevStatus) {
      app.timeline.push({
        activity: `Loan sanction status changed to "${req.body.sanctionStatus}"`,
        activityType: 'note',
        performedBy: req.user._id,
        previousStatus: prevStatus,
        newStatus: req.body.sanctionStatus,
        isSystemGenerated: true,
      });
    }

    await app.save();
    return ApiResponse.success(res, 'Loan sanction saved', app.loanSanction);
  } catch (err) {
    next(err);
  }
};
